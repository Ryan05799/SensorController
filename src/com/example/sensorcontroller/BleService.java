...
public class BleService extends Service{
	...
	private final class BleGattCallback extends BluetoothGattCallback{
		...
		public void onCharacteristicWrite(BluetoothGatt gatt, 
											BluetoothGattCharacteristic characteristic, int status){
			if(status == BluetoothGatt.GATT_SUCCESS){
				broadcastWriteSuccess(ACTION_CHARACTERISTIC_WRITE, gatt.getDevice().getName(), characteristic);
			}else{
	...
	private void broadcastWriteSuccess(final String action, String devName, 
											BluetoothGattCharacteristic characteristic){
		final Intent intent = new Intent(action);
        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
		intent.putExtra(DEVICE_NAME, devName);
